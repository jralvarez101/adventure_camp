class Campsites {
  ...
  async getCampground(stateSearch) {
    const campgroundResponse = await fetch(
      `https://developer.nps.gov/api/v1/campgrounds?parkCode=&stateCode=${stateSearch}&limit=${this.limit}&q=camping&api_key=${this.api_key}`
    );
