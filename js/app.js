...
function paginationInformation(campsiteData) {
  ...
  const currentCampsites = campsiteData.data.slice(
    indexOfFirstCampsite,
    indexOfLastCampsite
  );
...
async function initMap(currentCampsiteList, stateSearch) {
  if (currentCampsiteList === undefined) {
    ...
    const selectedStateCoordinates = stateCoordinates.find(
      (x) => x.state === stateSearch
    );
    ...
    function addMarker() {
      for (let i = 0; i < currentCampsiteList.length; i++) {
        ...
        // marker.addListener('click', function(campsiteId){
        //     return function(marker.id){
        //         console.log(marker.id);
        //     }
        // })

        marker.addListener("click", allCampsiteIds(marker.id));

        function allCampsiteIds(id) {
          return function thisMarkerId(id) {
...
document
  .getElementById("campsiteName")
  .addEventListener("blur", validateCampsite);
